...
import { deleteLink, insertLink, queryLinks } from '../../../api/src/lib/db/queries/link.table';
import { deleteImage, insertImage, queryImages } from '../../../api/src/lib/db/queries/image.table';
import { deleteAsset, insertAsset, queryAssets } from '../../../api/src/lib/db/queries/asset.table';
import { ImageTable } from '../../../api/src/models/types/tables/image-table';
import { updateContractDescription } from '../../../api/src/lib/db/queries/contract-metadata.table';
import { LSP4DigitalAssetMetadata } from '../../../api/src/lib/UniversalProfile/models/lsp4-digital-asset.model';
import { AssetTable } from '../../../api/src/models/types/tables/asset-table';
import { INDEX_DATA } from '../config';
import { Link } from '../../../api/src/models/types/metadata-objects';
...
export async function updateLSP4Metadata(address: string, lsp4: LSP4DigitalAssetMetadata) {
  ...
  try {
    ...
    const imagesToDelete = images.filter(
      (i) =>
        !lsp4.images
          .flat()
          .map((x) => x.hash)
          .includes(i.hash) && !lsp4.icon.map((x) => x.hash).includes(i.hash),
    );
    const imagesToAdd = lsp4.images
      ? lsp4.images.flat().filter((i) => !images.map((x) => x.hash).includes(i.hash))
      : [];
    const iconsToAdd = lsp4.icon
      ? lsp4.icon.filter((i) => !images.map((x) => x.hash).includes(i.hash))
      : [];
    ...
    const assetsToAdd = lsp4.assets
      ? lsp4.assets.filter((i) => !assets.map((x) => x.hash).includes(i.hash))
      : [];
    ...
    const linksToAdd = lsp4.links
      ? lsp4.links.filter((i) => !links.map((x) => x.url).includes(i.url))
      : [];
