...
import { LSP3UniversalProfile } from '../../../api/src/lib/UniversalProfile/models/lsp3-universal-profile.model';
...
import { deleteTag, insertTag, queryTags } from '../../../api/src/lib/db/queries/tag.table';
import {
  updateContractDescription,
  updateContractName,
} from '../../../api/src/lib/db/queries/contract-metadata.table';
...
export async function updateLSP3Profile(address: string, lsp3: LSP3UniversalProfile) {
  ...
  try {
    ...
    const imagesToDelete = images.filter(
      (i) =>
        !lsp3.profileImage.map((x) => x.hash).includes(i.hash) &&
        !lsp3.backgroundImage.map((x) => x.hash).includes(i.hash),
    );
    const profileImagesToAdd = lsp3.profileImage
      ? lsp3.profileImage.filter((i) => !images.map((x) => x.hash).includes(i.hash))
      : [];
    const backgroundImagesToAdd = lsp3.backgroundImage
      ? lsp3.backgroundImage.filter((i) => !images.map((x) => x.hash).includes(i.hash))
      : [];
    ...
    const linksToAdd = lsp3.links
      ? lsp3.links.filter((i) => !links.map((x) => x.url).includes(i.url))
      : [];
