...
import { queryContract } from '../../api/src/lib/db/queries/contract.table';
import { extractContract } from './extraction/extract-contract';
import { ContractFullMetadata } from '../types/contract-metadata';
import { queryTransaction } from '../../api/src/lib/db/queries/transaction.table';
import { Transaction } from '../../api/src/models/types/transaction';
...
import { queryEventByTh } from '../../api/src/lib/db/queries/event.table';
import { extractAndIndexDataChangedEvent } from './extraction/extract-data-changed';
import { POST_VALIDATOR_ADDRESS } from '../../api/src/environment/config';
...
import { MethodParameterTable } from '../../api/src/models/types/tables/method-parameter-table';
...
async function extractAndIndexLog(log: Log, lastBlock: number) {
  ...
  try {
    ...
    try {
      ...
      if (
        !(
          POST_VALIDATOR_ADDRESS.includes(event.params['to']) ||
          (event.params['dataKey'] && event.params['dataKey'] === KEY_LSPXXSocialRegistry)
        )
      ) {
