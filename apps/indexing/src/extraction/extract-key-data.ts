...
async function extractAndIndexLSP4Symbol(
  log: Log,
  lastBlock: number,
  erc725Y: ERC725,
  key: string,
  ...
  if (value && log.blockNumber > lastBlock - 10) {
    const symbol: string = erc725Y.decodeData(
      [{ value: [{ key, value }], keyName: 'LSP4TokenSymbol' }],
...
async function extractAndIndexLSP4Name(
  log: Log,
  lastBlock: number,
  erc725Y: ERC725,
  key: string,
  ...
  // We want to fetch the latest value if we have no value (obviously) OR if the log is old (meaning the value probably changed in the meantime)
  if (value && log.blockNumber > lastBlock - 10) {
    const name: string = erc725Y.decodeData(
      [{ value: [{ key, value }], keyName: 'LSP4TokenName' }],
      LSP4DigitalAssetJSON as ERC725JSONSchema[],
    ).value as string;
...
async function extractAndIndexLSP4Metadata(
  log: Log,
  lastBlock: number,
  key: string,
  erc725Y: ERC725,
  ...
  if (value && log.blockNumber > lastBlock - 10) {
    const decoded = ERC725.decodeData(
      [{ value: [{ key, value }], keyName: 'LSP4Metadata' }],
      LSP4DigitalAssetJSON as ERC725JSONSchema[],
    );
...
async function extractAndIndexLSP3Profile(
  log: Log,
  lastBlock: number,
  key: string,
  erc725Y: ERC725,
  ...
  // We want to fetch the latest value if we have no value (obviously) OR if the log is old (meaning the value probably changed in the meantime)
  if (value && log.blockNumber > lastBlock - 10) {
    const decodedJsonUrl = ERC725.decodeData(
      [{ value: [{ key, value }], keyName: 'LSP3Profile' }],
...
export async function extractDataFromKey(log: Log, lastBlock: number, key: string, value?: string) {
  const erc725Y = new ERC725(
    (LSP4DigitalAssetJSON as ERC725JSONSchema[]).concat(
      LSP3UniversalProfileMetadataJSON as ERC725JSONSchema[],
    ),
    log.address,
