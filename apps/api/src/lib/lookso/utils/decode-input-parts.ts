...
import { queryMethodInterface } from '../../db/queries/method-interface.table';
import { queryMethodParameters } from '../../db/queries/method-parameter.table';

const methods: {
  interface: Omit<MethodInterfaceTable, 'hash' | 'type'>;
  parameters: Omit<MethodParameterTable, 'methodId' | 'indexed'>[];
}[] = [
  {
...
export type DecodedFunctionCall = {
  contract: Contract;
  methodInterface: Omit<MethodInterfaceTable, 'hash' | 'type'>;
 ...
 */
export async function decodeInputParts(
  input: string,
  address: string,
  results: DecodedFunctionCall[],
  ...
  switch (input.slice(0, 10)) {
    case methods[0].interface.id: // execute method on LSP6 Key Manager
      ...
      upContractAddress = await new web3.eth.Contract(
        LSP6KeyManager.abi as AbiItem[],
        address,
      ).methods.target.call();
      return await decodeInputParts(
        decodedParams['payload'] as string,
        upContractAddress,
        results.concat(functionCall),
      );
    ...
    case methods[1].interface.id: // execute method on ERC725X contract
      ...
      return await decodeInputParts(
        decodedParams['data'] as string,
        web3.utils.toChecksumAddress(decodedParams['to']),
        results.concat(functionCall),
      );
        ...
        functionCall = {
          ...
          decodedParameters: methodParameters.map((m) => {
            return {
              name: m.name,
              type: m.type,
              value: decodedParams[m.name] !== null ? decodedParams[m.name].toString() : 'null',
            };
          }),
        };
      } catch (e) {
