...
import { queryRegistryChangesOfAddress } from '../../db/queries/registry-change.table';
import { generateNewRegistry, getRegistryUpdatesToPush } from './utils/generate-new-registry';
import { insertLike, querySenderLikes, removeLike } from '../../db/queries/like.table';
import { insertFollow, queryFollowing, removeFollow } from '../../db/queries/follow.table';
...
export async function applyChangesToRegistry(address: string): Promise<SocialRegistry> {
  ...
  for (const following of toPush.toAdd.follows)
    await tryExecuting(insertFollow(address, following));
  for (const hash of toPush.toRemove.likes) await tryExecuting(removeLike(address, hash));
  for (const following of toPush.toRemove.follows)
    await tryExecuting(removeFollow(address, following));
