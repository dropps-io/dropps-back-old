...
describe('GET lookso/post/:hash/likes', () => {
  ...
  it('should return the right amount of likes', async () => {
    expect(payload.results.length).toEqual(3);
  });
  ...
  it('should return the unnamed at the end', async () => {
    expect(payload.results[payload.results.length - 1].address).toEqual(UNIVERSAL_PROFILE_2);
  });

  it('should work with sender', async () => {
    res = await fastify.inject({
      method: 'GET',
      url: `/lookso/post/${POST_HASH}/likes?sender=${SERIOUS_MAN_UP}`,
    });
    payload = JSON.parse(res.payload);
    ...
    res = await fastify.inject({
      method: 'GET',
      url: `/lookso/post/${POST_HASH}/likes?sender=${UNIVERSAL_PROFILE_3}`,
    });
    payload = JSON.parse(res.payload);
    expect(payload.results.length).toEqual(0);
  });
  ...
  it('should return following status with viewOf', async () => {
    ...
    res = await fastify.inject({
      method: 'GET',
      url: `/lookso/post/${POST_HASH}/likes?viewOf=${HACKER_MAN_UP}`,
    });
    payload = JSON.parse(res.payload);
    ...
    expect(payload.results.filter((f) => f.address === UNIVERSAL_PROFILE_2)[0].following).toEqual(
      false,
    );
  });
  ...
  it('should return 400 if invalid viewOf address', async () => {
    res = await fastify.inject({
      method: 'GET',
      url: `/lookso/post/${POST_HASH}/likes?viewOf=${HACKER_MAN_UP}q`,
    });
    expect(res.statusCode).toEqual(400);
  });
  ...
  it('should return 400 if invalid sender address', async () => {
    res = await fastify.inject({
      method: 'GET',
      url: `/lookso/post/${POST_HASH}/likes?sender=${HACKER_MAN_UP}q`,
    });
    expect(res.statusCode).toEqual(400);
  });
  ...
  describe('With pagination', () => {
    ...
    it('should return the next page when not in the query', async () => {
      ...
      expect(payload.next).toEqual(`${API_URL}/lookso/post/${POST_HASH}/likes?page=1`);
    });
    ...
    it('should return the next page', async () => {
      const res = await fastify.inject({
        method: 'GET',
        url: `/lookso/post/${POST_HASH}/likes?page=1`,
      });
      payload = JSON.parse(res.payload);
      expect(payload.next).toBeNull();
      expect(payload.previous).toEqual(`${API_URL}/lookso/post/${POST_HASH}/likes?page=0`);
    });
    ...
    it('should return only one post on the last page if 61 posts in the feed', async () => {
      ...
      const res = await fastify.inject({
        method: 'GET',
        url: `/lookso/post/${POST_HASH}/likes?page=2`,
      });
      payload = JSON.parse(res.payload);
      expect(payload.next).toBeNull();
      expect(payload.previous).toEqual(`${API_URL}/lookso/post/${POST_HASH}/likes?page=1`);
