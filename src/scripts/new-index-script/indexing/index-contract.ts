...
export async function indexContract(address: string, code: string | null, metadata: ContractFullMetadata | null, exists?: boolean) {
	...
	if (metadata) {
		try {
			await insertContractMetadata(address, metadata.name, metadata.symbol, metadata.description, metadata.isNFT, '');
		} catch (e) {
			await reportIndexingScriptError('indexContract:metadata', e);
		}
		// Mapping used in each for loop to avoid duplicates
		if (metadata.assets) for (const asset of [...(new Map(metadata.assets.map((m) => [m.url, m]))).values()]) {
			try {
		...
		if (metadata.images) for (const image of [...(new Map(metadata.images.map((m) => [m.url, m]))).values()]) {
			try {
		...
		if (metadata.links) for (const link of [...(new Map(metadata.links.map((m) => [m.url, m]))).values()]) {
			try {
		...
		if (metadata.icon) for (const icon of [...(new Map(metadata.icon.map((m) => [m.url, m]))).values()]) {
			try {
		...
		if (metadata.backgroundImage) for (const image of [...(new Map(metadata.backgroundImage.map((m) => [m.url, m]))).values()]) {
			try {
		...
		if (metadata.profileImage) for (const image of [...(new Map(metadata.profileImage.map((m) => [m.url, m]))).values()]) {
			try {
		...
		if (metadata.tags) for (const tag of [...new Set(metadata.tags)]) {
			try {
		...
		if (metadata.avatar) try {
			await insertAsset(address, metadata.avatar.url, metadata.avatar.fileType, metadata.avatar.hash);
		} catch (e) {
