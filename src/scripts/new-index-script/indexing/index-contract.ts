...
export async function indexContract(address: string, code: string | null, metadata: ContractFullMetadata | null) {
  ...
  if (metadata) {
    ...
    if (metadata.assets) for (let asset of [...(new Map(metadata.assets.map((m) => [m.url, m]))).values()]) {
      try {
    ...
    if (metadata.images) for (let image of [...(new Map(metadata.images.map((m) => [m.url, m]))).values()]) {
      try {
    ...
    if (metadata.links) for (let link of [...(new Map(metadata.links.map((m) => [m.url, m]))).values()]) {
      try {
    ...
    if (metadata.icon) for (let icon of [...(new Map(metadata.icon.map((m) => [m.url, m]))).values()]) {
      try {
    ...
    if (metadata.backgroundImage) for (let image of [...(new Map(metadata.backgroundImage.map((m) => [m.url, m]))).values()]) {
      try {
    ...
    if (metadata.profileImage) for (let image of [...(new Map(metadata.profileImage.map((m) => [m.url, m]))).values()]) {
      try {
    ...
    if (metadata.tags) for (let tag of [...new Set(metadata.tags)]) {
      try {
