...
export async function fourBytesDictionaryFetch() {
  let next, n = 0;
  do {
    const methodInterfaces: any = await axios.get(next ? next : 'https://www.4byte.directory/api/v1/signatures/?ordering=created_at?page=1600');
    ...
    for (const result of (methodInterfaces.data.results as any[])) {
      try {
        ...
        for (const param of extractedMethod.params) {
          await insertMethodParameter(result.hex_signature, '', param);
        }
      } catch (e) {
        console.error(e);
      }
