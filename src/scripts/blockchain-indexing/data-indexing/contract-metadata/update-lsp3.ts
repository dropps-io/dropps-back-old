...
export async function updateLSP3Profile(address: string, lsp3: LSP3UniversalProfile) {
  ...
  try {
    ...
    for (let image of profileImagesToAdd) await tryExecuting(insertImage(address, image.url, image.width, image.height, 'profile', image.hash));
    for (let image of backgroundImagesToAdd) await tryExecuting(insertImage(address, image.url, image.width, image.height, 'background', image.hash));
