...
export function decodeSetDataValueFromInput(input: string): {key: string, value: string}[] {
  switch (input.slice(0, 10)) {
    case '0x09c5eabe':
      return decodeSetDataValueFromInput(web3.eth.abi.decodeParameters([{name: 'bytes', type: 'bytes'}], input.slice(10))['bytes'] as string);
    case '0x44c028fe':
      return decodeSetDataValueFromInput(web3.eth.abi.decodeParameters(['uint256', 'address', 'uint256' ,{name: 'bytes', type: 'bytes'}], input.slice(10))['bytes'] as string);
    case '0x902d5fa0':
      return decodeSetDataValueFromInput(web3.eth.abi.decodeParameters(['bytes', 'uint256',{name: 'bytes', type: 'bytes'}], input.slice(10))['bytes'] as string);
    case '0x7f23690c':
