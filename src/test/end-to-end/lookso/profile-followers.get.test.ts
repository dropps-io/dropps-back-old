  ...
  describe('GET lookso/profile/:address/followers', () => {
    ...
    beforeEach(async () => {
      ...
      res = await fastify.inject({
        method: 'GET',
        url: `/lookso/profile/${HACKER_MAN_UP}/followers`,
      });
    ...
    it('should return the right followers names and profile images', async () => {
      expect(payload.results.filter((f) => f.address === SERIOUS_MAN_UP)[0].name).to.equal(
        'SeriousMan',
      );
      expect(payload.results.filter((f) => f.address === SERIOUS_MAN_UP)[0].image).to.equal('url');
      expect(payload.results.filter((f) => f.address === UNIVERSAL_PROFILE_1)[0].name).to.equal(
        'UniversalProfile1',
      );
      expect(payload.results.filter((f) => f.address === UNIVERSAL_PROFILE_1)[0].image).to.equal(
        'url1',
      );
    });
    ...
    it('should work with follower address', async () => {
      res = await fastify.inject({
        method: 'GET',
        url: `/lookso/profile/${HACKER_MAN_UP}/followers?follower=${SERIOUS_MAN_UP}`,
      });
      payload = JSON.parse(res.payload);
      ...
      res = await fastify.inject({
        method: 'GET',
        url: `/lookso/profile/${HACKER_MAN_UP}/followers?follower=${UNIVERSAL_PROFILE_3}`,
      });
      payload = JSON.parse(res.payload);
    ...
    it('should return following status with viewOf', async () => {
      res = await fastify.inject({
        method: 'GET',
        url: `/lookso/profile/${HACKER_MAN_UP}/followers?viewOf=${HACKER_MAN_UP}`,
      });
      payload = JSON.parse(res.payload);
      expect(payload.results.filter((f) => f.address === SERIOUS_MAN_UP)[0].following).to.equal(
        true,
      );
      expect(
        payload.results.filter((f) => f.address === UNIVERSAL_PROFILE_1)[0].following,
      ).to.equal(false);
    });
    ...
    it('should return 400 if invalid address', async () => {
      res = await fastify.inject({
        method: 'GET',
        url: `/lookso/profile/${HACKER_MAN_UP}q/followers`,
      });
    ...
    it('should return 400 if invalid viewOf address', async () => {
      res = await fastify.inject({
        method: 'GET',
        url: `/lookso/profile/${HACKER_MAN_UP}/followers?viewOf=${HACKER_MAN_UP}q`,
      });
      expect(res.statusCode).to.equal(400);
    });
    ...
    it('should return 400 if invalid follower address', async () => {
      res = await fastify.inject({
        method: 'GET',
        url: `/lookso/profile/${HACKER_MAN_UP}/followers?follower=${HACKER_MAN_UP}q`,
      });
      expect(res.statusCode).to.equal(400);
    });
    ...
    describe('With pagination', () => {
      ...
      it('should return the next page when not in the query', async () => {
        ...
        expect(payload.next).to.equal(
          `${API_URL}/lookso/profile/${HACKER_MAN_UP}/followers?page=1`,
        );
      });
      ...
      it('should return the next page', async () => {
        const res = await fastify.inject({
          method: 'GET',
          url: `/lookso/profile/${HACKER_MAN_UP}/followers?page=1`,
        });
        payload = JSON.parse(res.payload);
        expect(payload.next).to.be.null;
        expect(payload.previous).to.equal(
          `${API_URL}/lookso/profile/${HACKER_MAN_UP}/followers?page=0`,
        );
      });
      ...
      it('should return only one post on the last page if 61 posts in the feed', async () => {
        ...
        const res = await fastify.inject({
          method: 'GET',
          url: `/lookso/profile/${HACKER_MAN_UP}/followers?page=2`,
        });
        payload = JSON.parse(res.payload);
        expect(payload.next).to.be.null;
        expect(payload.previous).to.equal(
