...
describe('GET lookso/profile/:address/activity', () => {
  ...
  beforeEach(async () => {
    ...
    res = await fastify.inject({
      method: 'GET',
      url: `/lookso/profile/${HACKER_MAN_UP}/activity?viewOf=${SERIOUS_MAN_UP}`,
    });
  ...
  it('should work with post filter', async () => {
    ...
    res = await fastify.inject({
      method: 'GET',
      url: `/lookso/profile/${HACKER_MAN_UP}/activity?postType=post`,
    });
    payload = JSON.parse(res.payload);
    expect(payload.results.length).toEqual(2);
  ...
  it('should work with event filter', async () => {
    ...
    res = await fastify.inject({
      method: 'GET',
      url: `/lookso/profile/${HACKER_MAN_UP}/activity?postType=event`,
    });
    payload = JSON.parse(res.payload);
  ...
  it('should return 400 if invalid address', async () => {
    res = await fastify.inject({
      method: 'GET',
      url: `/lookso/profile/${HACKER_MAN_UP}q/activity`,
    });
  ...
  describe('With pagination', () => {
    ...
    it('should return the previous page when not in the query', async () => {
      res = await fastify.inject({
        method: 'GET',
        url: `/lookso/profile/${HACKER_MAN_UP}/activity`,
      });
      payload = JSON.parse(res.payload);
      expect(payload.next).toBeNull();
      expect(payload.previous).toEqual(
        `${API_URL}/lookso/profile/${HACKER_MAN_UP}/activity?page=0`,
      );
    });
    ...
    it('should return the next page', async () => {
      res = await fastify.inject({
        method: 'GET',
        url: `/lookso/profile/${HACKER_MAN_UP}/activity?page=0`,
      });
      payload = JSON.parse(res.payload);
      expect(payload.previous).toBeNull();
      expect(payload.next).toEqual(`${API_URL}/lookso/profile/${HACKER_MAN_UP}/activity?page=1`);
    });

    it('should return the next page with query params', async () => {
      res = await fastify.inject({
        method: 'GET',
        url: `/lookso/profile/${HACKER_MAN_UP}/activity?viewOf=${SERIOUS_MAN_UP}&postType=post&page=0`,
      });
      payload = JSON.parse(res.payload);
      expect(payload.previous).toBeNull();
      expect(payload.next).toEqual(
        `${API_URL}/lookso/profile/${HACKER_MAN_UP}/activity?postType=post&viewOf=${SERIOUS_MAN_UP}&page=1`,
      );
    });
    ...
    it('should return the previous page with query params', async () => {
      res = await fastify.inject({
        method: 'GET',
        url: `/lookso/profile/${HACKER_MAN_UP}/activity?viewOf=${SERIOUS_MAN_UP}&postType=post`,
      });
      payload = JSON.parse(res.payload);
      expect(payload.next).toBeNull();
      expect(payload.previous).toEqual(
        `${API_URL}/lookso/profile/${HACKER_MAN_UP}/activity?postType=post&viewOf=${SERIOUS_MAN_UP}&page=0`,
      );
    });
    ...
    it('should return only one post on the last page if 61 posts in the feed', async () => {
      ...
      expect(payload.next).toBeNull();
      expect(payload.previous).toEqual(
        `${API_URL}/lookso/profile/${HACKER_MAN_UP}/activity?page=1`,
      );
