...
export const FeedGETTests = () => {
  ...
  describe('GET lookso/feed', () => {
    ...
    beforeEach(async () => {
      ...
      await insertPost(POST_HASH2, SERIOUS_MAN_UP, new Date('2022-09-27T12:03:32.089Z'), 'test1', '', null, null, null);
    ...
    it('should work with post filter', async () => {
      ...
      await insertPost(POST_HASH3, SERIOUS_MAN_UP, new Date('2022-09-27T12:03:33.089Z'), 'test1', '', null, null, id);
    ...
    describe ('With pagination', () => {
      ...
      it ('should return the previous page when not in the query', async () => {
        ...
        expect(payload.previous).to.equal(API_URL + '/lookso/feed?page=0');
      });
      ...
      it ('should return only one post on the last page if 61 posts in the feed', async () => {
        ...
        payload = JSON.parse(res.payload);
        expect(payload.next).to.be.null;
        expect(payload.previous).to.equal(API_URL + '/lookso/feed?page=1');
