...
export const FeedGETTests = () => {
  ...
  describe('GET lookso/feed', () => {
    ...
    beforeEach(async () => {
      ...
      await insertPost(POST_HASH, HACKER_MAN_UP, new Date(), 'test', '', null, null, null);
      await insertPost(POST_HASH2, SERIOUS_MAN_UP, new Date(), 'test1', '', null, null, null);
    ...
    it('should work with limit', async () => {
      res = await fastify.inject({method: 'GET', url: `/lookso/feed?limit=1`});
      feed = JSON.parse(res.payload);
      expect(feed.length).to.equal(1);
    });

    it('should work with offset', async () => {
      res = await fastify.inject({method: 'GET', url: `/lookso/feed?offset=1`});
      feed = JSON.parse(res.payload);
      expect(feed.length).to.equal(1);
    });
    ...
    it('should work with post filter', async () => {
      ...
      await insertPost(POST_HASH3, SERIOUS_MAN_UP, new Date(), 'test1', '', null, null, id);
