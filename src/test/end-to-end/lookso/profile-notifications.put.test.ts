...
import {HACKER_MAN_JWT, HACKER_MAN_UP, POST_HASH, SERIOUS_MAN_JWT, SERIOUS_MAN_UP, UNIVERSAL_PROFILE_1, UNIVERSAL_PROFILE_2} from "../../helpers/constants";
...
import {insertNotification, queryNotificationsOfAddress} from "../../../lib/db/queries/notification.table";
import {insertPost} from "../../../lib/db/queries/post.table";


export const ProfileNotificationsPUTTests = () => {
  ...
  describe('PUT lookso/profile/:address/notifications', () => {
    ...
    it('should return 200', async () => {
      const res = await fastify.inject({
        method: 'PUT', url: `/lookso/profile/${HACKER_MAN_UP}/notifications`,
        headers: {
    ...
    it('should return the right amount of notifications', async () => {
      ...
      expect(notifications[0].viewed).to.equal(true);
      expect(notifications[1].viewed).to.equal(true);
      expect(notifications[2].viewed).to.equal(true);
    ...
    it('should return 400 if invalid address', async () => {
      const res = await fastify.inject({
        method: 'PUT', url: `/lookso/profile/${HACKER_MAN_UP}a/notifications`,
        headers: {
          authorization: 'Bearer ' + HACKER_MAN_JWT
        }});
