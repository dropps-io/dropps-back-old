...
describe('GET lookso/tx/:hash', () => {
  ...
  beforeEach(async () => {
    ...
    await insertContractMetadata(
      '0xE905EaceE9B020C8D024f25FCAd60Df0ED195064',
      'Dennis',
      '',
      '',
      false,
    ...
    await insertImage(
      '0xE905EaceE9B020C8D024f25FCAd60Df0ED195064',
      'ipfs://QmXojnbciwL1XVe23rFu59AHtqNZPgPMoSYqMtT3jCBHiz',
      300,
      300,
      'profile',
      '0x00',
    ...
    await insertContractMetadata(
      '0xe557CbF347EEF6575D04e412A052933F3bD84dF8',
    ...
    await insertImage(
      '0xe557CbF347EEF6575D04e412A052933F3bD84dF8',
      'ipfs://Qmam5zvRwdWruVssnb7sYhJkrNVguVR7yTpAJcmpk9TXJo',
      300,
      300,
      '',
      '0x00',
  ...
  it('should return the right tx data', async () => {
    expect(payload.hash).toEqual(expectedResponse.hash);
    expect(payload.value).toEqual(expectedResponse.value);
    expect(payload.blockNumber).toEqual(expectedResponse.blockNumber);
    expect(payload.from).toEqual(expectedResponse.from);
    expect(payload.to).toEqual(expectedResponse.to);
    expect(payload.input).toEqual(expectedResponse.input);
    expect(payload.methodId).toEqual(expectedResponse.methodId);
    expect(payload.decodedFunctionCallParts.length).toEqual(
      expectedResponse.decodedFunctionCallParts.length,
    );
  });

  it('should return the right decodedFunctionCallParts', async () => {
    expectedResponse.decodedFunctionCallParts.forEach((fnCall, i) => {
      expect(fnCall.contract.address).toEqual(payload.decodedFunctionCallParts[i].contract.address);
      expect(fnCall.contract.image).toEqual(payload.decodedFunctionCallParts[i].contract.image);
      expect(fnCall.contract.name).toEqual(payload.decodedFunctionCallParts[i].contract.name);
      expect(fnCall.contract.standard).toEqual(
        payload.decodedFunctionCallParts[i].contract.standard,
      );
      expect(fnCall.methodInterface.id).toEqual(
        payload.decodedFunctionCallParts[i].methodInterface.id,
      );
      expect(fnCall.methodInterface.name).toEqual(
        payload.decodedFunctionCallParts[i].methodInterface.name,
      );

      fnCall.decodedParameters.forEach((param, j) => {
        expect(param.value).toEqual(payload.decodedFunctionCallParts[i].decodedParameters[j].value);
        expect(param.type).toEqual(payload.decodedFunctionCallParts[i].decodedParameters[j].type);
        expect(param.value).toEqual(payload.decodedFunctionCallParts[i].decodedParameters[j].value);
      });
    });
  });
  ...
  it("should return unknown function if input can't be decoded", async () => {
    ...
    expect(payload.decodedFunctionCallParts[2]).toBeDefined();
    expect(payload.decodedFunctionCallParts[2].methodInterface.name).toEqual('Unknown Function');
    expect(payload.decodedFunctionCallParts[2].decodedParameters.length).toEqual(0);
  });

  it('should return 404 if tx not found', async () => {
    res = await fastify.inject({
      method: 'GET',
      url: `/lookso/tx/0xf000df1845cb8f5cdd78985049defa2571968d8302d39fa79e55fd6ed72b91c1`,
    });
