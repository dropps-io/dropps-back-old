...
export const TxGetTests = () => {
  describe('GET lookso/tx/:hash', () => {
    ...
    it('should return the right tx data', async () => {
      ...
      expect(payload.decodedFunctionCallParts.length).to.equal(
    ...
    it('should return the right decodedFunctionCallParts', async () => {
      expectedResponse.decodedFunctionCallParts.forEach((fnCall, i) => {
        expect(fnCall.contract.address).to.equal(
          payload.decodedFunctionCallParts[i].contract.address,
        );
        ...
        expect(fnCall.contract.standard).to.equal(
          payload.decodedFunctionCallParts[i].contract.standard,
        );
        expect(fnCall.methodInterface.id).to.equal(
          payload.decodedFunctionCallParts[i].methodInterface.id,
        );
        expect(fnCall.methodInterface.name).to.equal(
        ...
        fnCall.decodedParameters.forEach((param, j) => {
          expect(param.value).to.equal(
            payload.decodedFunctionCallParts[i].decodedParameters[j].value,
          );
          expect(param.type).to.equal(
            payload.decodedFunctionCallParts[i].decodedParameters[j].type,
          );
          expect(param.value).to.equal(
            payload.decodedFunctionCallParts[i].decodedParameters[j].value,
          );
