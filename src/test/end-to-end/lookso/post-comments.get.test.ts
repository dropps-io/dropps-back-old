...
describe('GET lookso/post/:hash/comments', () => {
  ...
  it('should return the right amount of comments', async () => {
    expect(payload.results.length).toEqual(4);
  });

  it('should return posts in chronological order', async () => {
    expect(payload.results[3].hash).toEqual(POST_HASH5);
    expect(payload.results[2].hash).toEqual(POST_HASH4);
    expect(payload.results[1].hash).toEqual(POST_HASH3);
    expect(payload.results[0].hash).toEqual(POST_HASH2);
  });
  ...
  it('should show if a post is liked or not if viewOf', async () => {
    ...
    expect(payload.results.filter((p) => p.hash === POST_HASH5)[0].isLiked).toEqual(false);
  });
  ...
  describe('With pagination', () => {
    ...
    it('should return the previous page when not in the query', async () => {
      ...
      expect(payload.previous).toEqual(`${API_URL}/lookso/post/${POST_HASH}/comments?page=0`);
    });

    it('should return the right amount of posts', async () => {
      res = await fastify.inject({ method: 'GET', url: `/lookso/post/${POST_HASH}/comments` });
      payload = JSON.parse(res.payload);
      expect(payload.results.length).toEqual(COMMENTS_PER_LOAD);
    });
    ...
    it('should return the next page', async () => {
      ...
      expect(payload.next).toEqual(`${API_URL}/lookso/post/${POST_HASH}/comments?page=1`);
    });
    ...
    it('should return only one post on the last page if 61 posts in the feed', async () => {
      ...
      payload = JSON.parse(res.payload);
      expect(payload.next).toBeNull();
      expect(payload.previous).toEqual(`${API_URL}/lookso/post/${POST_HASH}/comments?page=1`);
      expect(payload.count).toEqual(COMMENTS_PER_LOAD * 2 + 1);
