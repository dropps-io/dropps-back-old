    ...
    describe ('With pagination', () => {
      beforeEach( async () => {
        for (let i = 0 ; i < COMMENTS_PER_LOAD * 2 - 4 ; i++) {
          await insertPost(generateRandomKeccakHash(), SERIOUS_MAN_UP, new Date('2022-09-27T12:03:32.089Z'), 'test1', '', POST_HASH, null, null);
        }
      });
      ...
      it ('should return the next page', async () => {
        res = await fastify.inject({method: 'GET', url: `/lookso/post/${POST_HASH}/comments?page=0`});
        payload = JSON.parse(res.payload);
      ...
      it ('should return the next page with query params', async () => {
        res = await fastify.inject({method: 'GET', url: `/lookso/post/${POST_HASH}/comments?viewOf=${SERIOUS_MAN_UP}&page=0`});
        payload = JSON.parse(res.payload);
        expect(payload.previous).to.be.null;
        expect(payload.next).to.equal(`${API_URL}/lookso/post/${POST_HASH}/comments?viewOf=${SERIOUS_MAN_UP}&page=1`);
      });

      it ('should return the previous page with query params', async () => {
        res = await fastify.inject({method: 'GET', url: `/lookso/post/${POST_HASH}/comments?viewOf=${SERIOUS_MAN_UP}`});
        payload = JSON.parse(res.payload);
        expect(payload.next).to.be.null;
        expect(payload.previous).to.equal(`${API_URL}/lookso/post/${POST_HASH}/comments?viewOf=${SERIOUS_MAN_UP}&page=0`);
      });
