...
describe('POST lookso/profile/:address/registry', () => {
  ...
  it('should return a jsonUrl on correct request', async () => {
    ...
    expect(JSON.parse(res.payload).jsonUrl).toBeDefined();
  });
  ...
  it('should correctly update the database', async () => {
    ...
    expect(likes).toContain(POST_HASH2);
    expect(likes).not.toContain(POST_HASH);
    expect(follows).not.toContain(HACKER_MAN_UP);
    expect(follows).toContain(UNIVERSAL_PROFILE_1);
    expect(follows).not.toContain(SERIOUS_MAN_UP);
  });
