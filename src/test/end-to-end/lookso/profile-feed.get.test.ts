...
export const ProfileFeedGETTests = () => {
  ...
  describe('GET lookso/profile/:address/feed', () => {
    ...
    it('should return posts in chronological order', async () => {
      expect(payload.results[1].hash).to.equal(POST_HASH4);
      expect(payload.results[0].hash).to.equal(POST_HASH3);
    });
    ...
    describe ('With pagination', () => {
      ...
      it ('should return the previous page when not in the query', async () => {
        ...
        expect(payload.previous).to.equal(`${API_URL}/lookso/profile/${HACKER_MAN_UP}/feed?page=0`);
      });
      ...
      it ('should return the next page', async () => {
        res = await fastify.inject({method: 'GET', url: `/lookso/profile/${HACKER_MAN_UP}/feed?page=0`});
        payload = JSON.parse(res.payload);
        expect(payload.previous).to.be.null;
        expect(payload.next).to.equal(`${API_URL}/lookso/profile/${HACKER_MAN_UP}/feed?page=1`);
      });

      it ('should return the next page with query params', async () => {
        res = await fastify.inject({method: 'GET', url: `/lookso/profile/${HACKER_MAN_UP}/feed?postType=post&page=0`});
        payload = JSON.parse(res.payload);
        expect(payload.previous).to.be.null;
        expect(payload.next).to.equal(`${API_URL}/lookso/profile/${HACKER_MAN_UP}/feed?postType=post&page=1`);
      });

      it ('should return the previous page with query params', async () => {
        res = await fastify.inject({method: 'GET', url: `/lookso/profile/${HACKER_MAN_UP}/feed?postType=post&page=1`});
        payload = JSON.parse(res.payload);
        expect(payload.next).to.be.null;
        expect(payload.previous).to.equal(`${API_URL}/lookso/profile/${HACKER_MAN_UP}/feed?postType=post&page=0`);
      });
      ...
      it ('should return only one post on the last page if 61 posts in the feed', async () => {
        ...
        payload = JSON.parse(res.payload);
        expect(payload.next).to.be.null;
        expect(payload.previous).to.equal(`${API_URL}/lookso/profile/${HACKER_MAN_UP}/feed?page=1`);
