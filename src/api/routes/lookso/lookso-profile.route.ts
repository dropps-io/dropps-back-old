  ...
  fastify.route({
    ...
    handler: async (request, reply) => {
      ...
      try {
        ...
        const posts: Post[] = await queryPostsOfUser(
          address,
          POSTS_PER_LOAD,
          page * POSTS_PER_LOAD,
          query.postType,
    ...
    handler: async (request, reply) => {
      ...
      try {
        if (query.follower) {
          ...
          if (page >= count / PROFILES_PER_LOAD)
            return reply.code(400).send(error(400, ERROR_INVALID_PAGE));
          const followers = await queryFollowersWithNames(
            address,
            PROFILES_PER_LOAD,
            page * PROFILES_PER_LOAD,
          );
          for (const follower of followers) {
            ...
            const following = query.viewOf
              ? await queryFollow(query.viewOf, follower.address)
              : undefined;
            response.push({
              ...follower,
          ...
          const queryUrl = `${API_URL}/lookso/profile/${address}/followers?${
            query.follower ? 'follower=' + query.follower + '&' : ''
          }${query.viewOf ? 'viewOf=' + query.viewOf + '&' : ''}page=`;
          ...
          return reply.code(200).send({
            ...
            page,
            next:
              page < Math.ceil(count / PROFILES_PER_LOAD) - 1
    ...
    handler: async (request, reply) => {
      ...
      try {
        ...
        for (const followingProfile of following) {
          ...
          const selectedImage = selectImage(images, { minWidthExpected: 50 });
          const following = query.viewOf
            ? await queryFollow(query.viewOf, followingProfile.address)
            : undefined;
          response.push({
            ...followingProfile,
            image: selectedImage ? selectedImage.url : '',
  ...
  fastify.route({
    ...
    handler: async (request, reply) => {
      ...
      try {
        ...
        const page =
          query.page !== undefined ? query.page : Math.ceil(count / NOTIFICATIONS_PER_LOAD) - 1;
        if (page >= count / NOTIFICATIONS_PER_LOAD)
          return reply.code(400).send(error(400, ERROR_INVALID_PAGE));
        const notifications = await queryNotificationsOfAddress(
          address,
          NOTIFICATIONS_PER_LOAD,
          page * NOTIFICATIONS_PER_LOAD,
        );
        ...
        return reply.code(200).send({
          ...
          page,
          next:
            page < Math.ceil(count / NOTIFICATIONS_PER_LOAD) - 1
              ? queryUrl + (page + 1).toString()
              : null,
          previous: page > 0 ? queryUrl + (page - 1).toString() : null,
          results: response,
