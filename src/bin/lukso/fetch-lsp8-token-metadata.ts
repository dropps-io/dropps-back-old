 ...
 */
export const fetchLsp8TokenInfo = async (
  assetAddress: string,
  tokenId: string,
  ...
  return {
    ...
    image: tokenMetadata?.images
      ? formatUrl(selectImage(tokenMetadata.images, { minWidthExpected: 200 })?.url ?? '')
      : '',
 ...
 */
const fetchLsp8TokenMetadata = async (
  contractAddress: string,
  tokenId: string,
  tokenIdType: TOKEN_ID_TYPE,
): Promise<LSP4DigitalAssetMetadata | null> => {
  const erc725 = new ERC725(
    LSP8IdentifiableDigitalAssetSchema.concat(LSP4DigitalAssetSchema) as ERC725JSONSchema[],
    contractAddress,
  ...
  try {
    return ((await erc725.fetchData(tokenMetadataKey)).value as any)
      .LSP4Metadata as LSP4DigitalAssetMetadata;
  } catch (error) {
    logError(error);
