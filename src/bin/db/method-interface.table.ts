...
export async function queryMethodInterfacesByType(type: 'function' | 'event'): Promise<MethodInterfaceTable[]> {
	const res = await executeQuery('SELECT * FROM "method_interface" WHERE "type" = $1', [type]);
	return res.rows as MethodInterfaceTable[];
}

export async function insertMethodInterface(id: string, hash: string, name: string, type: 'function' | 'event'): Promise<MethodInterfaceTable> {
