...
export async function queryKeyDisplay(key: string): Promise<KeyDisplayTable> {
  const res = await executeQuery('SELECT * FROM "key_display" WHERE LOWER(key) LIKE LOWER($1)', [
    `%${key.slice(0, 26)}%`,
  ]);
...
export async function insertKeyDisplay(
  key: string,
  display: string,
  displayWithoutValue: string,
): Promise<void> {
  await executeQuery('INSERT INTO "key_display" VALUES ($1, $2, $3)', [
    key,
    display,
    displayWithoutValue,
  ]);
