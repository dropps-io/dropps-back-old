...
export function generateNewRegistry(decentralizedRegistry: SocialRegistry, changes: RegistryChangeTable[]): SocialRegistry {
	...
	changes.forEach(change => {
		if (change.type === 'like'){
			...
			if (change.action === 'add' && !decentralizedRegistry.follows.includes(change.value)) newRegistry.follows.push(change.value);
			else if (change.action === 'remove') {
...
export function getRegistryUpdatesToPush(centralizedRegistry: SocialRegistry, newDecentralizedRegistry: SocialRegistry): {toAdd: SocialRegistry, toRemove: SocialRegistry} {
	const toRemove: SocialRegistry = {
		posts: [],
		likes: centralizedRegistry.likes.filter(x => newDecentralizedRegistry.likes.indexOf(x) === -1),
		follows: centralizedRegistry.follows.filter(x => newDecentralizedRegistry.follows.indexOf(x) === -1),
	};

	const toAdd: SocialRegistry = {
		posts: [],
		likes: newDecentralizedRegistry.likes.filter(x => centralizedRegistry.likes.indexOf(x) === -1),
		follows: newDecentralizedRegistry.follows.filter(x => centralizedRegistry.follows.indexOf(x) === -1),
	};
