...
export class ArweaveClient {
  ...
  public async downloadJson(txId: string) {
    ...
    switch (response.status) {
      ...
      case 400 | 404:
        throw new Error(
          `https://arweave.net/${txId}` + ': Unable to download. Transaction not found',
        );
      default:
  ...
  private async arToDollar(valueInAR: string): Promise<string> {
    ...
    try {
      response = await fetch(
        `https://pro-api.coinmarketcap.com/v2/tools/price-conversion?amount=${valueInAR}&id=${this.ARID}`,
        { headers },
      );
    } catch (error: any) {
