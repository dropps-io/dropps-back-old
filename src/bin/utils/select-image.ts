...
export function selectImage(images: ImageTable[] | (MetadataImage | MetadataImage[])[], config: {minWidthExpected: number}) {
	const sortedImages = images.flat().sort((i1, i2) => {
		if (i1.width > i2.width) return -1;
		else return 1;
	});

	let pickedImage: ImageTable | MetadataImage = sortedImages[0];
