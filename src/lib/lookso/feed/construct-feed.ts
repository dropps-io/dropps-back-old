import { queryDecodedEventParameters } from '../../db/queries/decoded-event-parameter.table';
import { queryEvent } from '../../db/queries/event.table';
import { Post } from '../../../models/types/post';
import { DecodedParameter } from '../../../models/types/decoded-parameter';
import { EventTable } from '../../../models/types/tables/event-table';
import {
  generateDataChangedDisplay,
  generateEventDisplay,
  generateUniversalReceiverEventDisplay,
} from './generate-event-display';
import { queryDecodedFunctionParameters } from '../../db/queries/decoded-function-parameter.table';
import { queryContractName } from '../../db/queries/contract-metadata.table';
import { queryImagesByType } from '../../db/queries/image.table';
import { selectImage } from '../../utils/select-image';
import { queryPostLike, queryPostLikesCount } from '../../db/queries/like.table';
import {
  queryPost,
  queryPostCommentsCount,
  queryPostRepostsCount,
} from '../../db/queries/post.table';
import { FeedPost } from '../../../models/types/feed-post';

export async function constructFeed(
  posts: Post[],
  profile?: string,
  noRecursive?: boolean,
): Promise<FeedPost[]> {
  ...
  for (const post of posts) {
    ...
    const authorPic = selectImage(await queryImagesByType(post.author, 'profile'), {
      minWidthExpected: 50,
    });
    ...
    if (post.eventId) {
      ...
      const parameters: Map<string, DecodedParameter> = new Map(
        (await queryDecodedEventParameters(post.eventId)).map((x) => {
          return [x.name, x];
        }),
      );
      ...
      try {
        switch (event.topic) {
          ...
          case '0x4810874456b8e6487bd861375cf6abd8e1c8bb5858c8ce36a86a04dabfac199e': // Executed
            const transactionParameters: Map<string, DecodedParameter> = new Map(
              (await queryDecodedFunctionParameters(event.transactionHash)).map((x) => {
                return [x.name, x];
              }),
            );
            const [selector, executionContract] = [
              parameters.get('selector'),
              parameters.get('to'),
            ];
            try {
              feedObject.display = await generateEventDisplay(
                selector ? selector.value : '',
                transactionParameters,
                {
                  executionContract: executionContract ? executionContract.value : '',
                  senderProfile: event.address,
                },
              );
            } catch (e) {
              feedObject.display = await generateEventDisplay(event.topic.slice(0, 10), parameters);
