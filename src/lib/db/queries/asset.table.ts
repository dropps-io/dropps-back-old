...
export async function queryAssetsByType(address: string, fileType: string): Promise<AssetTable[]> {
  const res = await executeQuery('SELECT * FROM "asset" WHERE "address" = $1 AND "fileType" = $2', [
    address,
    fileType,
  ]);
  if (res.rows.length > 0) return res.rows as AssetTable[];
...
export async function insertAsset(
  ...
  const res = await executeQuery('INSERT INTO "asset" VALUES ($1, $2, $3, $4)', [
    address,
    url,
    fileType,
    hash,
...
export async function deleteAsset(address: string, url: string): Promise<void> {
  const res = await executeQuery('DELETE FROM "asset" WHERE "address" = $1 AND "url" = $2', [
    address,
    url,
  ]);
