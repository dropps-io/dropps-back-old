  ...
  fastify.route({
    ...
    handler: async (request, reply) => {
      ...
      try {
        ...
        try {
          response.decodedFunctionCallParts = await decodeInputParts(tx.input, tx.to ? tx.to : '', []);
        } catch (e) {}
        ...
        for (let i = 0 ; i < response.decodedFunctionCallParts.length ; i++) {
          try {
            ...
            const images = contract.interfaceCode === 'LSP0' ? await queryImagesByType(address, 'profile') : await queryImages(address);
            response.decodedFunctionCallParts[i].contract = {address, standard: contract.interfaceCode ? contract.interfaceCode : undefined, name, image: selectImage(images, {minWidthExpected: 210}).url}
          } catch (e) {}
        }
